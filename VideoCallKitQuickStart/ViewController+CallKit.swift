...
extension ViewController2 : CXProviderDelegate {
    ...
    func providerDidReset(_ provider: CXProvider) {
        ...
        CallManager.shared.room?.disconnect()
    }
    ...
    func provider(_ provider: CXProvider, perform action: CXStartCallAction) {
        ...
        CallManager.shared.callKitProvider.reportOutgoingCall(with: action.callUUID, startedConnectingAt: nil)
...
extension ViewController2 {
    ...
    func performStartCallAction(uuid: UUID, roomName: String?) {
        ...
        CallManager.shared.callKitCallController.request(transaction)  { error in
            if let error = error {
    ...
    func reportIncomingCall(uuid: UUID, roomName: String?, completion: ((NSError?) -> Void)? = nil) {
        ...
        CallManager.shared.callKitProvider.reportNewIncomingCall(with: uuid, update: callUpdate) { error in
            if error == nil {
    ...
    func performRoomConnect(uuid: UUID, roomName: String? , completionHandler: @escaping (Bool) -> Swift.Void) {
        ...
        if !CallManager.shared.isCallInProgress {
            ...
            let connectOptions = TVIConnectOptions.init(token: accessToken) { (builder) in
                ...
                builder.audioTracks = CallManager.shared.localAudioTrack != nil ? [CallManager.shared.localAudioTrack!] : [TVILocalAudioTrack]()
                builder.videoTracks = CallManager.shared.localVideoTrack != nil ? [CallManager.shared.localVideoTrack!] : [TVILocalVideoTrack]()
            ...
            CallManager.shared.room = TwilioVideo.connect(with: connectOptions, delegate: self)
        }
        ...
        CallManager.shared.callKitCompletionHandler = completionHandler
    }
}
